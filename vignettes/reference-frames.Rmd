...
Since `shift_reference_frame()` doesn't modify the underlying physics — it just translates coordinates — you can call it multiple times on the same simulation to explore different viewpoints. There's no need to re-run `simulate()`.
