...
- **Save and share** — `save_system()` / `load_system()` round-trip a system to `.rds`; `export_bodies()` writes the body table to CSV
...
- **[Keplerian Orbital Elements](https://orbit-r.com/articles/keplerian-elements.html)** — the physics behind `add_body_keplerian()` and `add_planet()`
