...
### `plot_orbits(sim_data, force_3d = FALSE)`

A smart plotting dispatcher that automatically chooses between 2D and 3D visualization. If any body has non-zero Z positions (or if `force_3d = TRUE`), it renders an interactive 3D plot using `plotly`. Otherwise it produces a 2D trajectory map (x vs y) using `ggplot2` with `coord_equal()`.

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `sim_data` | `tibble` | — | Output from `simulate()` |
| `force_3d` | `logical` | `FALSE` | Force 3D rendering even for planar data |
...
You can also force 3D rendering for planar data with `force_3d = TRUE`, which can be useful if you want the interactive rotation and zoom capabilities even for a flat system.
...
The same three-body system from earlier, but with the Moon's orbital inclination included:
