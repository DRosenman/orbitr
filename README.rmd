...
The same three-body system from earlier, but with the Moon's orbital inclination included. Note the use of `keep_center = FALSE` to remove Earth from the plot and `dplyr::filter()` to drop the Sun — without this, the Sun's enormous apparent orbit (~150 billion m) dwarfs the Moon's trajectory (~384 million m) and you can't see anything useful:
